...
SELECT a.account_id,
       ...
       CASE WHEN u.profile_image IS NULL THEN 0 ELSE 1 END AS has_profile_image,
       CASE WHEN u.profile_image IS NULL THEN '' ELSE ISNULL(CONVERT(varchar(33), u.profile_image_updated_at_utc, 126), '1') END AS profile_image_code,
       p.last_seen_utc
FROM dbo.Accounts a
...
SELECT TOP(@Max)
    ...
    ) THEN 1 ELSE 0 END AS has_pending_outgoing,
    CASE WHEN EXISTS (
        SELECT 1 FROM dbo.FriendRequests fr
        WHERE fr.status = 0 AND fr.from_user_id = a.account_id AND fr.to_user_id = @Me
    ) THEN 1 ELSE 0 END AS has_pending_incoming,
    (
        SELECT TOP(1) fr.friend_request_id
        FROM dbo.FriendRequests fr
        WHERE fr.status = 0 AND fr.from_user_id = a.account_id AND fr.to_user_id = @Me
        ORDER BY fr.sent_at DESC
    ) AS pending_incoming_request_id
FROM dbo.Accounts a
LEFT JOIN dbo.Users u ON u.user_id = a.account_id
...
WHERE a.account_id = @Id;";
            ...
            public const string GET_PROFILE_IMAGE = @"
SELECT u.profile_image,
       u.profile_image_content_type,
       u.profile_image_updated_at_utc
FROM dbo.Users u
WHERE u.user_id = @Id;";
        }
