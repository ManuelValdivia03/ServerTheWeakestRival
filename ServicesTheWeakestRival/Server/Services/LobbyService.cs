...
namespace ServicesTheWeakestRival.Server.Services
{
    ...
    public sealed class LobbyService : ILobbyService
    {
        ...
        public void LeaveLobby(LeaveLobbyRequest request)
        {
            ...
            {
                if (request != null && request.LobbyId != Guid.Empty && userId > INVALID_USER_ID)
                {
        ...
        public JoinByCodeResponse JoinByCode(JoinByCodeRequest request)
        {
            ...
            if (string.IsNullOrWhiteSpace(request.AccessCode))
            {
                throw LobbyServiceContext.ThrowFault(
                    LobbyServiceConstants.ERROR_INVALID_REQUEST,
                    LobbyServiceConstants.MESSAGE_ACCESS_CODE_REQUIRED);
            }
        ...
        private void EnsureInitialized()
        {
            if (lobbyRepository != null
                && lobbyRoomOperations != null
                && lobbyAccountOperations != null
                && lobbyMatchOperations != null)
            {
                return;
            }
            ...
            {
                lobbyMatchOperations = new LobbyMatchOperations(
                    connectionString,
                    callbackHub,
                    lobbyRepository,
                    CreateAvatarSql);
        ...
        private static string ResolveConnectionString(string name)
        {
            ...
            if (setting == null || string.IsNullOrWhiteSpace(setting.ConnectionString))
            {
                throw new InvalidOperationException(
                    string.Format("Missing connectionString '{0}' in App.config.", name));
            }
